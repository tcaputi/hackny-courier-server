...
var config = {
  'secrets' : {
    'clientId' : '2ZRRG4ROIOKMSOPKFQGEARAMUGNASI5XLOJ0YMDNMYFJVUXB',
    'clientSecret' : 'SSDBGFQHBUUGIH3BRZR2EFW31ZEY2NMN5W3NHGNSHN3D5COQ',
    'redirectUrl' : 'http://sandile.me:7373/callback'
  }
};
...
$(function() {
	...
	messageContainer.bind('keypress', function(e) {
		...
		else 
		{
			navigator.geolocation.getCurrentPosition(function(position){
				for(var i=0; i<places.length(); i++){
					if(message.content.indexOf(places[i]) != -1) {
						request('https://api.foursquare.com/v2/venues/explore?ll="' + message.location.lat + ',' + message.location.long + '"&radius=15000&query=' + places[i], function (error, response, body) {
							if (!error && response.statusCode == 200) {
								socket.emit('message', {from:"pseudo", location:{lat: position.coords.latitude, long: position.coords.longitude}, content: messageContainer.val(), payload: body});
...
function addMessage(msg, pseudo, date, self) {
	...
	$("#chatEntries").append('<div class="'+classDiv+'"><p class="infos"><span class="pseudo">'+pseudo+'</span>, <time class="date" title="'+date+'">'+date+'</time></p><p>' + msg.content + '</p></div>');
	time();
}
